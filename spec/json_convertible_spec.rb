...
class MockEmptyJSONConvertible
...
module JSONConvertible
  describe JSONConvertible do
    let (:mock_empty_object) { MockEmptyJSONConvertible.new }
    ...
    it "Serializes empty classes back and forward from JSON" do
      json_from_mock_object = mock_empty_object.to_object_dictionary

      expect(json_from_mock_object).to eql({})

      object = MockEmptyJSONConvertible.from_json!({})

      expect(object.instance_variables.length).to eql(0)
    end
    ...
    it "Raises exception when required initialization parameters are not found" do
      ...
      expect { MockJSONConvertibleWithRequiredParams.from_json!(object_dictionary) }.to(raise_exception(JSONConvertibleError))
    end
